...
index_meetcyclus <- derive_index_meetcyclus_inla(analyseset_species_locations,
                             indexmodel_doyrw2_iid_meetcyclus)
...
trend_offset <- derive_trend(analyseset_gb, trendmodel_offset) %>%
   mutate(model_offset = "offset",
         model_locatie = "subgebied/plot",
         waic = trendmodel_offset$waic$waic)
...
trend_geen_offset <- derive_trend(analyseset_gb, trendmodel_geen_offset) %>%
   mutate(model_offset = "geen offset",
         model_locatie = "subgebied/plot",
         waic = trendmodel_geen_offset$waic$waic)
...
data_predict <- data.frame(jaar_centered = c(0:5,NA)) %>%
  mutate(jaar = jaar_centered + 2016)

data_combine <- bind_rows(analyseset_gb %>%
                            mutate(data_type = "obserevd"),
                          data_predict %>%
                            mutate(data_type = "predict"))

check <- analyseset_gb %>%
  group_by(jaar) %>%
  summarise(mean_prop = mean(prop_aanwezig_plant),
            var_prop = var(prop_aanwezig_plant),
            prob = sum(n_plant_metei)/sum(n_plant)) %>%
  ungroup()
...
prob_rw1 <- data_combine %>%
  mutate(mean = plogis(model_propplanten_rw1$summary.fitted.values$mean),
         lcl_0.95 = plogis(model_propplanten_rw1$summary.fitted.values$`0.025quant`),
         ucl_0.95 = plogis(model_propplanten_rw1$summary.fitted.values$`0.975quant`)) %>%
  filter(data_type == "predict")

model_propplanten_jaarfactor <- inla(n_plant_metei ~ 0 +  fjaar  + 
                             f(subgebied, model = "iid", 
                               hyper = list(theta = list(prior = "pc.prec", param = c(1, 0.05)))) +
                             f(plot, model = "iid", 
                               hyper = list(theta = list(prior = "pc.prec", param = c(1, 0.05)))) ,
...
summary(model_propplanten_jaarfactor)


model_inla.samples <- inla.posterior.sample(1000, model_propplanten_rw1)
  
sample <- model_inla.samples[[1]]

  quantile_values <- c(0.025, 0.05, 0.20, 0.35,  0.65, 0.80, 0.95, 0.975)
  
   fun = function(...) {
      c(plogis(fjaar2016), plogis(fjaar2017),  plogis(fjaar2018), plogis(fjaar2019), plogis(fjaar2020), plogis(fjaar2021))
    }
  
  predict_year_inla <- inla.posterior.sample.eval(fun, model_inla.samples) %>%
    as.data.frame() %>%
    mutate(jaar = (min(analyseset_gb$jaar)):max(analyseset_gb$jaar)) %>%
    ...
    mutate(soort_nl = unique(analyseset_gb$soort_nl),
           soort_wet = unique(analyseset_gb$soort_wet),
           ref_jaar = min(jaar) - 1,
           parameter = "index") %>%
...
```{r}
sigma_rw_est <- model_propplanten_rw1$marginals.hyperpar$`Precision for jaar_centered` %>%
  inla.emarginal(fun = to_sigma)

model_propplanten_rw1$summary.random$jaar_centered %>%
  select(ID, mean, lcl = `0.025quant`, ucl = `0.975quant`) %>%
  mutate_at(c("mean", "lcl", "ucl"), exp) %>%
  ggplot(aes(x= ID, y = mean, ymin = lcl, ymax = ucl)) +
  geom_ribbon(alpha = 0.2) + 
...
```{r}
model_trendprop_jaarfactor <- inla(n_plant_metei ~ 0 +  jaar_centered  + 
                             f(subgebied, model = "iid", 
                               hyper = list(theta = list(prior = "pc.prec", param = c(1, 0.05)))) +
                             f(plot, model = "iid", 
                               hyper = list(theta = list(prior = "pc.prec", param = c(1, 0.05)))) ,
...
summary(model_trendprop_jaarfactor)
```
