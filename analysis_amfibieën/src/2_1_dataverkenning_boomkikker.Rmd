...
```{r}
overzicht_larven %>%
  select(cluster, poel, "2017 - 2019", "2020 - 2022") %>%
  arrange(cluster, poel) %>%
  kable(caption = "Aantal fuiktellingen per cyclus") %>%
  kable_styling() %>%
  collapse_rows(1)
...
```{r}
datasetset_boomkikker <- datasetset_boomkikker %>%
  filter(jaar <= 2021)
```
...
```{r}
mean_sample <- datasetset_boomkikker %>%
  filter(is_sample) %>%
  filter(primaire_soort) %>%
...
mean_larve_10schep<- datasetset_boomkikker %>%
  filter(primaire_soort) %>%
  filter(is_sample) %>%
  filter(levensstadium == "larve") %>%
  filter(!is.na(n_catch)) %>%
