...
```{r}
soortgroep_sel <- c("vogels", "Vogels")
```


## Meetnetkarakteristieken

Tabel \@ref(tab:MeetnKaraktabv1) geeft een overzicht van de karakteristieken van de Algemene Broedvogelmonitoring Vlaanderen (ABV).
...
tabel_meetnetkarakteristieken %>%
  ...
  unique() %>%
  arrange(meetnet) %>%
  kable(caption = "Overzicht van de karakteristieken van ABV: veldprotocol, type meetnet (integraal of steekproef), aantal meetnetlocaties en de meetcyclus in jaren",
...
In Tabel \@ref(tab:overzichtTellingenabv1) evalueren we de jaardoelen. In de tabel onderscheiden we: 
...
In Tabel \@ref(tab:overzichtTellingenabv3) evalueren we de doelen voor de meetcyclus. De tabel geeft volgende informatie: 
...
Figuur \@ref(fig:figuurBezoekenabv) geeft een overzicht van de tijdstippen van de bezoeken.  

```{r figuurBezoekenabv, fig.cap = str_c("Overzicht tijdstippen waarop tellingen werden uitgevoerd in ", year_evaluation) }
...
```{r}
bezoeken_buiten_telperiode_abv <- bezoeken_buiten_telperiode %>%
  filter(soortgroep %in% soortgroep_sel) %>%
  select(-soortgroep, -protocol)
```

`r if (nrow(bezoeken_buiten_telperiode_abv) > 0) {"Onderstaande tabel geeft een overzicht van de tellingen buiten de telperiode. De kolom 'verschil_dagen' geeft het verschil in dagen t.o.v. de telperiode volgens het veldprotocol."} else {"Er zijn geen bezoeken buiten de telperiode."}`

```{r}
if (nrow(bezoeken_buiten_telperiode_abv) > 0) {

bezoeken_buiten_telperiode_abv %>%
  filter(soortgroep %in% soortgroep_sel) %>%
  select(-soortgroep, -protocol) %>%
