...
In Tabel \@ref(tab:overzichtTellingenoverig3) evalueren we de doelen voor de meetcyclus. De tabel geeft volgende informatie: 
...
overzicht_prioritair %>%
  arrange(desc(jaren)) %>%
  datatable(caption = "Overzicht prioritair te bezoeken locaties (waarvoor aanwezigheid nog niet met zekerheid werd vastgesteld)",
            rownames = FALSE,
            filter = "top")
```
```{r}
overzicht_overige %>%
  arrange(desc(jaren)) %>%
  datatable(caption = "Overzicht overige bezochte locaties",
            rownames = FALSE,
            filter = "top")
```
