...
derive_trend_prop <- function(sample) {
  ...
  s <- sample$latent %>%
    ...
    mutate(prop_1 = plogis(`(Intercept)` + 1 * year_scaled),
           prop_4 = plogis(`(Intercept)` + 4 * year_scaled),
           trend_total_1_4 = (prop_4 / prop_1 - 1) * 100,
           trend_average_1_4 = ((trend_total_1_4/100 + 1) ^ (1/3) - 1) * 100) %>%
    pivot_longer(cols = c("(Intercept)", "year_scaled", "prop_1", "prop_4", "trend_total_1_4", "trend_average_1_4"), names_to = "parameter",
                 values_to = "value")
  ...
  overzicht_trend_tabel_nestbuizen <- estimates_trend_prop %>%
    filter(parameter %in% c("trend_total_1_4", "trend_average_1_4")) %>%
    mutate( n_jaar = 4,
         treshold_low = ifelse(parameter == "trend_average_1_4", 
                               round((exp(log(0.75)/(n_jaar - 1)) - 1) * 100, 1),
                               -25),
         treshold_high = ifelse(parameter == "trend_average_1_4", 
  ...
  mutate(periode = "2020 - 2023",
         soort_nl = "Hazelmuis") %>%
  select(parameter, soort_nl, periode, klasse, trend_tbl) %>%
  spread(key = "parameter", value = "trend_tbl") %>%
  mutate(klasse = as.character(klasse)) %>%
  select("Nederlandse naam" = soort_nl,  Periode = periode, Klasse = klasse,  "Jaarlijkse wijziging" = trend_average_1_4, "Wijziging over de looptijd" = trend_total_1_4) %>%
