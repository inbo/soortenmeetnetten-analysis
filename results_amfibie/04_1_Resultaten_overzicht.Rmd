...
verschillen_jaren %>%
  select(-soort_wet) %>%
  rename("Nederlandse naam" = soort_nl, Levensstadium = levensstadium,  Referentiejaar = ref_jaar) %>%
...
```{r fighk1, fig.cap = "Proportie van de waterpartijen met aanwezigheid van vis en met een permanente waterkolom en 90% betrouwbaarheidsinterval"}
habitatkenmerken_numeric_status %>%
 ...
 # stat_sum(alpha = 0.6) +
  stat_summary(fun.data = "mean_cl_boot", fun.args = list(conf.int = .9), colour = "red", size = 1) +
...
```{r fighk3, fig.width = 11, fig.height= 7, fig.cap= "Proportie van de plassen en totaal aantal plassen voor de verschillende categorieën van de habitatkenmerken waterkwaliteit, maximale diepte, beschaduwing en oppervlakte plas"}
