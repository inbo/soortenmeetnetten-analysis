...
versie <- str_c(name_analysis2, "_2025-12-18")
...
aantallen_regio <- analyseset_meetcyclus_argusvlinder_regio %>%
  group_by(meetnet, regio, meetcyclus, periode) %>%
...
indexmodel_doyrw2_iid_meetcyclus_KP <- inla(aantal ~ periode  + 
                              ...
                              family = "nbinomial",
                              data = analyseset_meetcyclus_argusvlinder_regio %>%
                                filter(regio == "Kustduinen en polders"),
                              offset = log_section_100,
                              control.compute = list(config = TRUE, waic = TRUE),
                              control.predictor = list(compute = TRUE))
...
aantallen_regio <- analyseset_meetcyclus_heivlinder_regio %>%
  group_by(meetnet, regio, meetcyclus, periode) %>%
  summarise(aantal_mean = mean(aantal),
            aantal_tot = sum(aantal)) %>%
  ungroup()
...
versie <- str_c(name_analysis0, "_2025-12-18")
