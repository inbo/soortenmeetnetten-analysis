...
data_sim_doy_add <- analyseset_adult %>%
  as_tibble() %>%
  tidyr::expand(meetcyclus,  doy = full_seq(doy, 1)) %>%
  mutate(doy_scaled = doy - min(doy) + 1,
         meetcyclus_start = start_cyclus + (meetcyclus - 1) * duur_cyclus,
         meetcyclus_end = meetcyclus_start + duur_cyclus - 1,
         periode = factor(str_c(meetcyclus_start, " - ", meetcyclus_end))) %>%
  mutate(data_type = "simulated_doy")

data_combine <- analyseset_adult %>%
...
summary(indexmodel_iid_doyrw2_bis)
```
...
data_predict <- data_combine %>%
  ...
  left_join(select(analyseset_adult, aantal_obs = aantal, doy, periode), by = c("doy", "periode")) 
...
+ bij gemodeleerde seizonaliteit bevat betrouwbaarheidsinterval van relatief effect steeds 1 (geen relatief verchil in aantallen tussen dagen)
...
data_sim_cluster_add <- analyseset_adult %>%
  as_tibble() %>%
  tidyr::expand(meetcyclus,  cluster) %>%
  mutate( meetcyclus_start = start_cyclus + (meetcyclus - 1) * duur_cyclus,
         meetcyclus_end = meetcyclus_start + duur_cyclus - 1,
         periode = factor(str_c(meetcyclus_start, " - ", meetcyclus_end))) %>%
...
trendmodel_iid <- inla(formula = aantal ~ meetcyclus  + 
                                f(locatie, 
                                  model = "iid", 
                                  hyper = list(theta = list(prior = "pc.prec", param = c(1, 0.05))))  + 
                                f(cluster,
...
trendmodel_iid2 <- inla(formula = aantal ~ meetcyclus  + 
                                f(locatie, 
                                  model = "iid", 
                                  hyper = list(theta = list(prior = "pc.prec", param = c(1, 0.05))))  + 
                                f(cluster,
...
test <- indexmodel_interaction$summary.fixed

indexmodel_iid2$summary.random$cluster2
```
...
data_predict_random_slope %>%
  left_join(select(analyseset_adult, aantal_obs = aantal, cluster, periode), by = c("cluster", "periode")) %>%
...
analyseset_adult <- analyseset_adult %>%
