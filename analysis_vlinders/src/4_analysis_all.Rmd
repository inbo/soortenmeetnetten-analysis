...
analyseset_transecten_locations <- analyseset_transecten_locations %>%
...
index_jaar_oz <- results_indexmodel %>%
  filter(meetnet == "Oranje zandoogje",
         parameter == "index") %>%
...
index_jaar_oz %>%
  write_csv2("../output/index_jaar.csv",na = "")

index_meetcyclus_oz_ref <- results_indexmodel_meetcyclus %>%
   filter(soort_nl == "Oranje zandoogje",
         parameter == "index") %>% 
  distinct(parameter, soort_nl, soort_wet) %>%
  mutate(meetcyclus = "2017-2019",
         meetcyclus_ref = "2017-2019",
         mean = 1)

index_meetcyclus_oz <- results_indexmodel_meetcyclus %>%
  filter(soort_nl == "Oranje zandoogje",
         parameter == "index") %>%
  select(parameter, soort_nl, soort_wet, meetcyclus, meetcyclus_ref, mean, lcl_0.90, ucl_0.90) %>%
  mutate(meetcyclus = "2020-2022",
         meetcyclus_ref = "2017-2019") %>%
  bind_rows(index_meetcyclus_oz_ref) %>%
  arrange(meetcyclus) %>%
...
index_meetcyclus_oz %>%
  write_csv2("../output/index_meetcyclus.csv")
```
...
models_inla <- analyseset_locations_by_regio %>%
  mutate(indexmodel = pmap(list(data, offset_var), fit_indexmodel_rw_nbinom_inla),
         trendmodel = map2(data, offset_var, fit_trendmodel_rw_nbinom_inla))

results_indexmodel_argusvlinder_regio <- models_inla %>%
  transmute(index = pmap(list(data, indexmodel, use_seed), derive_index_rw_inla)) %>%
  unnest(cols = c(meetnet, index))

results_trendmodel_argusvlinder_regio  <- models_inla %>%
  transmute(trend = map2(data, trendmodel, derive_trend)) %>%
...
ggsave("../output/figures/2024/verschil_meetcyclus_argusvlinder.jpg",
       width = 5, height = 4)
```
```{r}
verschil_meetcyclus_regio <- results_index_meetcyclus_av_regio %>%
  mutate(klasse = classification(lcl_0.90, ucl_0.90, threshold = c(0.75, 1.33), reference = 1),
         klasse = format(klasse, type = "markdown"),
         meetcyclus = str_remove(meetcyclus, "meetcyclus"),
         meetcyclus = str_replace(meetcyclus, "_", "-")) %>%
...
ggsave("../output/figures/2024/verschil_meetcyclus_argusvlinder_regio.jpg",
       width = 7, height = 4)
```
