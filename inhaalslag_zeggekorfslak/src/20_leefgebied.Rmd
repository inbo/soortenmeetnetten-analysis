...
In het monitorngsprotocol (Packet et al., 2018) wordt het potentieel leefgebied als volg afgeleid:
...
# bwk_codes_leefgebied2 <- c(" mc | mr | vn | vm | md | hfc ")
...
selectie_leefgebied_extra <- habitatmap_leefgebied_basis_ruim %>%
  st_drop_geometry() %>%
  separate_rows(bwk_label, sep =  " \\+ ") %>%
...
# leefgebied <- bind_rows(habitatmap_leefgebied,
#                         habitatmap_rbbmc_polygons,
#                         habitatmap_leefgebied_extra) %>%
#   st_drop_geometry() %>%
#   select(polygon_id, type_leefgebied)
  
```


Onderstaande tabel geeft een overzicht van de bwk-codes die geselecteerd worden en het aantal polygonen per bwk-code binnen het potentieel leefgebied van de Zeggekorfslak. 
...
We werken verder met twee varianten van het potentieel leefgebied:
...
```{r}
data_zeggekorfslak_bwkcode <- data_zeggekorfslak_sf %>%
...
area_leefgebied <- tibble(
  type_leefgebied = c("leefgebied", "leefgebied_ruim", "rbbmc"),
...
selectie_prioritair <- locaties_prioritair_area_leefgebied %>%
  ...
  select(locatie, "oppervlakte leefgebied ruim (ha)" = area_leefgebied_ruim_ha, "oppervlakte leefgebied strikt (ha)" = area_leefgebied_ha, "oppervlakte rbbmc (ha)" = area_rbbmc_ha) 
