...
In dit onderdeel geven we een voerzicht van de getelde aantallen.

```{r}

counts <- get_counts_smp("libellen", count_aggregation = "lifestage") # 'imago' and 'imago (uitgekleurd)' are agregated
...
summary_table %>% 
  ...
  kable(caption = "Overzicht getelde aantallen voor de prioritaire soorten") %>%
...
summary_counts_type %>% 
  ...
  kable(caption = "Overzicht getelde aantallen voor de prioritaire soorten met onderscheid tussen geslacht, levensstadium en activiteit") %>%
  kable_styling(bootstrap_options = c( "condensed"), font_size = 10) %>%
  column_spec(2, italic = TRUE) %>%
  collapse_rows(c(1,2, 3), target = 1) %>%
  scroll_box(width = "100%", height = "600px", fixed_thead = TRUE)
...
summary_table_all_years %>% 
  ...
  kable(caption = "Totaal aantal getelde overige soorten voor alle libellenmeetnetten samen") %>%
  kable_styling(bootstrap_options = c( "condensed"), font_size = 10) %>%
  column_spec(2, italic = TRUE) %>%
  collapse_rows(c(1,2, 3), target = 1) %>%
  scroll_box(width = "100%", height = "600px", fixed_thead = TRUE)
