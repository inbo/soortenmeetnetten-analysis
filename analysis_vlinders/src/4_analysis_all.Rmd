...
versie <- "vlinders_imago_transect_2025-11-03"
...
index_meetcyclus_oz <- derive_index_meetcyclus_inla(analyseset_meetcyclus_oz, 
                                                    function_eval = function(...) {c(exp(meetcyclus2020_2022))},
                             indexmodel_doyrw2_iid_meetcyclus, set_seed = seed)
...
```
```{r}
results_indexmodel <- read_vc(root = "../output/temp", file = "results_indexmodel_vlinders_imago_transect")
results_indexmodel_meetcyclus <- read_vc(root = "../output/temp", file = "results_indexmodel_meetcyclusvlinders_imago_transect")

index_jaar <- results_indexmodel %>%
  filter(meetnet %in% c("Heivlinder", "Argusvlinder", "Aardbeivlinder"),
         parameter == "index") %>%
  select( parameter, soort_nl, soort_wet, jaar, ref_jaar, mean, lcl_0.90, ucl_0.90) %>%
  mutate(mean = ifelse(is.na(mean), 1, mean),
         mean = round(mean, 3),
         lcl_0.90 = round(lcl_0.90, 3),
         ucl_0.90 = round(ucl_0.90, 3))

index_jaar %>%
  write_csv2("../output/index_jaar_vlinders.csv",na = "")

index_meetcyclus_vlinders_ref <- results_indexmodel_meetcyclus %>%
   filter(soort_nl %in% c("Argusvlinder", "Heivlinder"),
         parameter == "index") %>% 
  distinct(parameter, soort_nl, soort_wet) %>%
  mutate(meetcyclus = "2016-2018",
         meetcyclus_ref = "2016-2018",
         mean = 1)

index_meetcyclus_vlinders <- results_indexmodel_meetcyclus %>%
  filter(soort_nl %in% c("Argusvlinder", "Heivlinder"),
         parameter == "index") %>%
  select(parameter, soort_nl, soort_wet, meetcyclus, meetcyclus_ref, mean, lcl_0.90, ucl_0.90) %>%
  mutate(meetcyclus = str_remove(meetcyclus, "meetcyclus"),
         meetcyclus = str_replace(meetcyclus, "_", "-"),
         meetcyclus_ref = "2016-2018") %>%
  bind_rows(index_meetcyclus_vlinders_ref) %>%
  arrange(meetcyclus) %>%
...
index_meetcyclus_vlinders %>%
  write_csv2("../output/index_meetcyclus_vlinders.csv",na = "")
```
...
analyseset_meetcyclus_argusvlinder_regio <- analyseset_meetcyclus_argusvlinder %>%
...
results_indexmodel_argusvlinder_regio <- models_inla_regio %>%
  transmute(index = pmap(list(data, indexmodel, use_seed), derive_index_rw_inla)) %>%
  unnest(cols = c(meetnet, index))
...
indexmodel_doyrw2_iid_meetcyclus_AH <- inla(aantal ~ meetcyclus  + 
                                f(doy_scaled2, model = "rw2", 
                                  hyper = list(theta = list(prior = "pc.prec", param = c(0.01, 0.05)))) + 
                                 f(locatie, model = "iid", 
                                   hyper = list(theta = list(prior = "pc.prec", param = c(1, 0.05)))),
...
indexmodel_doyrw2_iid_meetcyclus_KP <- inla(aantal ~ meetcyclus  + 
                                f(doy_scaled2, model = "rw2", 
                                  hyper = list(theta = list(prior = "pc.prec", param = c(0.01, 0.05)))) + 
                                 f(locatie, model = "iid", 
                                   hyper = list(theta = list(prior = "pc.prec", param = c(1, 0.05)))),
...
analyseset_meetcyclus_heivlinder_kust <- analyseset_meetcyclus_heivlinder %>%
  left_join(locaties_hv, by = c("meetnet", "locatie")) %>%
  filter(provincie == "West-Vlaanderen")

indexmodel_hv_kust <- inla(aantal ~ meetcyclus  + 
                                f(doy_scaled2, model = "rw2", 
                                  hyper = list(theta = list(prior = "pc.prec", param = c(0.01, 0.05)))) + 
                                 f(locatie, model = "iid", 
                                   hyper = list(theta = list(prior = "pc.prec", param = c(1, 0.05)))),
...
analyseset_meetcyclus_heivlinder_antw_lim <- analyseset_meetcyclus_heivlinder %>%
  left_join(locaties_hv, by = c("meetnet", "locatie")) %>%
  filter(provincie != "West-Vlaanderen")

indexmodel_hv_antw_lim <- inla(aantal ~ meetcyclus  + 
                                f(doy_scaled2, model = "rw2", 
                                  hyper = list(theta = list(prior = "pc.prec", param = c(0.01, 0.05)))) + 
                                 f(locatie, model = "iid", 
                                   hyper = list(theta = list(prior = "pc.prec", param = c(1, 0.05)))),
...
index_meetcyclus_hv_regio <- index_meetcyclus_hv_antw_lim %>%
...
name_analyseset <- "analyseset_vlinders_imago_gebied"
versie <- "vlinders_imago_gebied_2025-11-03"
...
```{r, eval = FALSE}
verschil_jaren_ref_regio <- results_indexmodel_argusvlinder_regio %>%
  filter(parameter == "index") %>%
  filter(!is.na(mean)) %>%
  mutate(klasse = classification(lcl_0.90, ucl_0.90, threshold = c(0.75, 1.33), reference = 1),
         klasse = format(klasse, type = "markdown"))
  
verschil_jaren_ref_add <- verschil_jaren_ref_regio %>%
  distinct(soort_nl, soort_wet, regio, ref_jaar) %>%
  mutate(jaar = ref_jaar,
        mean = 1,
         klasse = "R") 

verschil_jaren_ref_regio <- verschil_jaren_ref_regio %>%
  bind_rows(verschil_jaren_ref_add) %>%
  mutate(mean_log = log(mean) + 1,
          lcl_0.90_log = log(lcl_0.90) + 1,
         ucl_0.90_log = log(ucl_0.90) + 1)
...
ggsave("../output/figures/2024/verschil_jaar_argusvlinder_regio.jpg",
       width = 7, height = 4)
```
...
```{r, eval = FALSE}
verschil_vorigjaar_regio <- results_indexmodel_argusvlinder_regio %>%
  filter(parameter == "diff_previous_year") %>%
  mutate(periode = str_c(lag(jaar), " - ", jaar )) %>%
  filter(!is.na(mean)) %>%
  mutate(klasse = classification(lcl_0.90, ucl_0.90, threshold = c(0.75, 1.33), reference = 1),
...
```{r}
verschil_meetcyclus %>%
  select(soort_nl, soort_wet, meetcyclus, klasse) %>%
  spread(key = "meetcyclus", value = "klasse") %>%
  rename("Nederlandse naam" = soort_nl) %>%
