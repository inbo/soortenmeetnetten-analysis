...
```{r}
visits <- get_visits_smp(species_group = species_group, file = "raw/bezoeken_20231204") %>%
...
covar <- read_vc(file = "covariabele_visit_20231204", root = file.path(path, "raw")) %>%
  mutate(visit_id = as.character(visit_id))

covar_sample <- read_vc(file = "covariabele_sample_20231204", root = file.path(path, "raw")) %>%
  mutate(visit_id = as.character(visit_id),
         sample_id = as.character(sample_id))

counts <- get_counts_smp(species_group = species_group, file = "raw/aantallen_20231204") %>%
  mutate(visit_id = as.character(visit_id))
...
analyseset_larven <- dataset_larven %>%
  ...
  ungroup() %>%
  select(soortgroep, meetnet, protocol, locatie, cluster, poel, is_sample, meetcyclus, meetcyclus_start, meetcyclus_end, jaar, datum, doy, visit_id, bezoek_status, zoekinspanning, larvetelling, metamorftelling, n_catch, primaire_soort, soort_nl, soort_wet, levensstadium, aantal, notes, hoofdteller)
```
...
analyseset_roepkoren_boomkikker <- dataset_roepkoren %>%
  ...
  mutate(meetcyclus_start = min(jaar),
         meetcyclus_end = min(jaar) + duur_cyclus - 1) %>%
  ungroup() %>%
  select(soortgroep, meetnet, protocol, locatie, cluster, poel, is_sample, meetcyclus, meetcyclus_start, meetcyclus_end, jaar, datum, visit_id, doy, bezoek_status, primaire_soort, soort_nl, soort_wet, levensstadium,  aantal, notes, hoofdteller)
```
...
analyseset_roepkoren_vroedmeesterpad <-  dataset_roepkoren %>%
  group_by(soortgroep, meetnet, protocol, locatie, is_sample, jaar, datum, visit_id, doy, bezoek_status, primaire_soort, soort_nl, soort_wet, levensstadium, notes, hoofdteller) %>%
  summarise(aantal = sum(aantal),
            n_points = n()) %>%
  ungroup() %>%
...
analyseset_roepkoren_vroedmeesterpad %>%
  select(meetnet, protocol, locatie, jaar, datum, aantal, notes) %>%
...
analyseset_fuiken_detail <- dataset_fuiken %>%
  ...
  ungroup() %>%
  select(soortgroep, meetnet, protocol, locatie, cluster, poel, is_sample, meetcyclus, meetcyclus_start, meetcyclus_end, jaar, datum, visit_id, doy, bezoek_status, n_fuiken, primaire_soort, soort_nl, soort_wet, levensstadium, geslacht, aantal, notes, hoofdteller)

analyseset_fuiken_totaal <- analyseset_fuiken_detail %>%
  group_by(soortgroep, meetnet, protocol, locatie, cluster, poel, is_sample, meetcyclus, meetcyclus_start, meetcyclus_end, jaar, datum, visit_id, doy, bezoek_status, n_fuiken, primaire_soort, soort_nl, soort_wet, notes, hoofdteller) %>%
  summarise(aantal = sum(aantal)) %>%
  ungroup() %>%
...
analyseset_rugstreeppad <- dataset_rugstreeppad %>%
  ...
  mutate(meetcyclus_start = min(jaar),
         meetcyclus_end = min(jaar) + duur_cyclus - 1) %>%
  ungroup() %>%
  select(soortgroep, meetnet, protocol, locatie, is_sample, meetcyclus, meetcyclus_start, meetcyclus_end, jaar, datum, visit_id, doy, bezoek_status, primaire_soort, soort_nl, soort_wet, levensstadium, activiteit, aantal, n_samples, notes, hoofdteller)
```
