...
In dit onderdeel geven we een overzicht van de resultaten van de analyses voor boomkikker, knoflookpad, kamsalamander en vuursalamander.
...
Daarom is het ook nuttig om periodes van drie jaar (die overeenkomen met een meetcyclus) met elkaar te vergelijken.
Voor boomkikker en kamsalamnder werden er al twee meetcycli afgerond en kunnen we dus deze beide periodes vergelijken (Tabel \@ref(tab:tabelverschil3)).
...
verschillen_cyclus <- result_indexmodel_meetcyclus %>%
  ...
  mutate(klasse = classification(lcl_0.90, ucl_0.90, threshold = c(0.75, 1.33), reference = 1)) %>%
  select(soort_nl, soort_wet, levensstadium, meetcyclus_ref, meetcyclus, klasse) %>%
  left_join(select(trend_klasses, klasse, klasse_output), by = "klasse") %>%
  select(-klasse)
...
verschillen_cyclus %>%
  ...
  rename("Nederlandse naam" = soort_nl, "Levensstadium" = levensstadium, "Meetcyclus 1" = meetcyclus_ref, "Meetcyclus 2" = meetcyclus, "Verschil" = klasse_output) %>%
...
overzicht_trend_tabel <- trend_all %>%
  ...
  select("Nederlandse naam" = soort_nl, Periode = periode_tekst, Klasse = klasse_output,"Type trend" = type_trend, "Jaarlijkse wijziging" = trend_average, "Wijziging over de looptijd" = trend_total)
...
Opnieuw maken we gebruik van de [classificatie](#h:classtrend) zoals besproken in paragraaf \@ref(h:classtrend) met -25 % als ondergrens en +33 % als bovengrens voor de totale trend over de meetperiode.
Voor een periode van zes jaar (2016 - 2021) komt de ondergrens overeen met een gemiddelde jaarlijkse trend van -5,6 % en de bovengrens met een gemiddelde jaarlijkse trend van +5,9 %.
...
```{r fighk1, fig.width = 7, fig.cap = "Proportie van de waterpartijen met aanwezigheid van vis en met een permanente waterkolom en 90 \\% betrouwbaarheidsinterval"}
