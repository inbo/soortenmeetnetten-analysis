...
-   de gegevens zijn afkomstig van meetnetlocaties (bijkomende tellingen op locaties die niet tot de steekproef behoren gebruiken we niet in de analyse omdat dit to vertekening kan leiden)

-   voor knoflookpad gebruiken we enkele de tellingen via een hydrofoon
...
Voor Boomkikker en Rugstreeppad voegen we ook een *random intercept* voor cluster toe, om aan te geven dat de aantallen per poelen binnen een cluster gecorreleerd zijn met elkaar.

De zoekingspanning per telling is verschillend bij de tellingen van vuursalamander, de larvetellingen en de fuiktellingen.
...
Hierdoor worden de resultaten uitgedrukt als respectievelijk het verschil in aantallen per 1000 meter transectlengte, het verschil in aantallen per 10 schepbewegingen en het verschil in aantallen per fuik.
...
-   $b_{c}$: een random intercept voor cluster $c$.
    Het volgt een normale distributie met gemiddelde 0 en variantie $\sigma_{c}^2$ (enkel bij boomkikker en kamsalmander)
...
Bij boomkikker en en kamsalamander voegen we een *random slope* voor cluster toe, zodat de lineaire trend kan variëren per cluster ($\beta_{1c}j$).
