...
locations <- get_locations_smp(only_active = FALSE) %>%
  ...
  mutate(is_sample = ifelse(locatie %in% c("Merkske - Poel B45", "Merkske - Poel B48", "Merkske - Poel B43"), TRUE, is_sample)) #aanpassen in meetnetten.be
...
dataset_roepkoren <-  visits %>%
  ...
  left_join(select(counts, visit_id, sample_id,  soort_nl, soort_wet, primaire_soort, levensstadium, aantal), by = "visit_id") 
```
...
analyseset_roepkoren_vroedmeesterpad <-  dataset_roepkoren %>%
  ...
  summarise(aantal = sum(aantal),
            n_points = n()) %>%
  ungroup() %>%
  filter(levensstadium != "onbekend")

analyseset_roepkoren_vroedmeesterpad %>%
  select(meetnet, protocol, locatie, jaar, datum, aantal, opmerking) %>%
...
counts_rugstreeppad_sample <- counts %>%
  ...
  group_by(visit_id, sample_id,  soort_nl, soort_wet, primaire_soort, levensstadium,  activiteit) %>%
  summarise(aantal = sum(aantal)) %>%
  ungroup()
...
analyseset_rugstreeppad <- dataset_rugstreeppad %>%
  ...
  mutate(meetcyclus_start = min(jaar),
         meetcyclus_end = min(jaar) + duur_cyclus - 1) %>%
  ungroup() %>%
  select(soortgroep, meetnet, protocol, locatie, is_sample, meetcyclus, meetcyclus_start, meetcyclus_end, jaar, datum, start_time, end_time, visit_id, doy, bezoek_status, primaire_soort, soort_nl, soort_wet, levensstadium, activiteit, aantal, n_samples, opmerking, hoofdteller)
...
# TE CORRIGEREN BEZOEK
analyseset_rugstreeppad <- analyseset_rugstreeppad %>%
  mutate(aantal = ifelse((visit_id == 55056) & (levensstadium == "larve"), 120, aantal),
         aantal = ifelse(visit_id == 55056 & activiteit == "roepend", 0, aantal))

check <- analyseset_rugstreeppad %>%
  filter(visit_id == "55056")
```
...
write_vc(analyseset_roepkoren_vroedmeesterpad, file = "analyseset_vroedmeesterpad", root = "../output/analyseset", sorting = c("meetnet", "locatie", "datum", "soort_nl", "visit_id"), strict = FALSE)
...
<!--chapter:end:2_9_dataverkenning_vroedmeesterpad.Rmd-->
...
indexmodel_iid_doyrw2_bis$summary.random$doy_scaled %>%
  select(doy_scaled = ID, mean, lcl = `0.025quant`, ucl = `0.975quant`) %>%
   mutate(doy = doy_scaled - 1 + doy_min,
         datum = as.Date(doy, origin = "2018-01-01")) %>%
  mutate(jaar = c(rep(2016, 47), rep(2017, 47), rep(2018, 47), rep(2019, 47), rep(2020, 47), rep(2021, 47))) %>%
  mutate_at(c("mean", "lcl", "ucl"), exp) %>%
  ggplot(aes(x = datum, y = mean, ymin = lcl, ymax = ucl)) +
  geom_ribbon(alpha = 0.2) + 
  geom_line() +
...
<!--chapter:end:4_1_analyse_pilootproject.Rmd-->
...
check <- analyseset_rugstreeppad_adult %>%
  distinct(type_telling, type_telling_est)
```
...
analyseset_rugstreeppad_adult <- analyseset_rugstreeppad_est %>%
  ...
  rename(ter_plaatse = "ter plaatse") %>%
  mutate(aantal_adult_roepend = roepend,
         aantal_adult_tot = roepend + ter_plaatse)
...
analyseset_rugstreeppad_adult %>%
  ...
  summarise(mean_puntlocaties = mean(n_samples),
            mean_roepend = mean(roepend),
            mean_roepend_per_punt = mean(roepend/n_samples),
            mean_terplaatse = mean(ter_plaatse),
            mean_terplaatse_punt = mean(ter_plaatse/n_samples)) %>%
  ungroup() %>%
...
<!--chapter:end:4_2_analyse_rugstreeppad.Rmd-->
...
indexmodel_jaar_rw1_iid_pertrend <- inla(formula = aantal ~ f(jaar_scaled, 
                                  model = "rw1", 
                                  hyper = list(theta = list(prior = "pc.prec", param = c(0.3, 0.05)))) +
                                f(locatie, 
                                  model = "iid", 
...
results_waic_boomkikker <- bind_rows(
  get_waic(analyseset_roepkoren_boomkikker_meetcyclus, trendmodel_iid2) %>%
    mutate(model = "trendmodel"),
  get_waic(analyseset_roepkoren_boomkikker_meetcyclus, indexmodel_jaar_rw1_iid_pertrend) %>%
...
trendmodel_iid2_2018_2023 <- inla(formula = aantal ~ year_scaled +
                                f(locatie,
                                  model = "iid",
                                  hyper = list(theta = list(prior = "pc.prec", param = c(1, 0.05)))) +
                                f(locatie2,
                       ...
                       family = "nbinomial",
                       offset = n_100m,
                       data = analyseset_vuursalamander_2018_2023,
                       control.compute = list(config = TRUE, waic = TRUE),
                       control.predictor = list(compute = TRUE))

indexmodel_iid_rw1year_2018_2023 <- inla(formula = aantal ~  f(jaar_scaled, 
                                  model = "rw1", 
                                  hyper = list(theta = list(prior = "pc.prec", param = c(0.3, 0.05)))) + 
                                f(locatie, 
                                  model = "iid", 
...
results_trend_vuursalamander_2018_2023 <- derive_trend(analyseset_species = analyseset_vuursalamander_2018_2023, trendmodel_nbinom = trendmodel_iid2_2018_2023)

results_waic_vuursalamander_2016_2023<- bind_rows(
  get_waic(analyseset_vuursalamander, trendmodel_iid2) %>%
    mutate(model = "trendmodel"),
  get_waic(analyseset_vuursalamander, indexmodel_iid_rw1year) %>%
    mutate(model = "indexmodel"))

results_waic_vuursalamander_2018_2023 <- bind_rows(
  get_waic(analyseset_vuursalamander_2018_2023, trendmodel_iid2_2018_2023) %>%
    mutate(model = "trendmodel"),
  get_waic(analyseset_vuursalamander_2018_2023, indexmodel_iid_rw1year_2018_2023) %>%
    mutate(model = "indexmodel")
  )
```
...
results_trend <- results_trend_kamsalamander %>%
  ...
  bind_rows(results_trend_vuursalamander_2018_2023) 
...
```{r}
 results_index_jaar %>%
  write_vc(root = "../output/results", file = "results_indexmodel_amfibieën", sorting = c("soort_nl", "levensstadium", "jaar", "parameter"), strict = FALSE)
```
```{r}
 results_index_meetcyclus %>%
  write_vc(root = "../output/results", file = "results_indexmodel_meetcyclus_amfibieën", sorting = c("soort_nl", "levensstadium", "meetcyclus"), strict = FALSE)
```
...
results_waic <- bind_rows(
  ...
  results_waic_vuursalamander_2016_2023 %>%
    mutate(trend_start = 2016),
  results_waic_vuursalamander_2018_2023 %>%
    mutate(trend_start = 2018)
...
results_waic %>%
  write_vc(root = "../output/results", file = "results_waic_amfibieën", sorting = c("soort_nl", "trend_start"), strict = FALSE)
```
