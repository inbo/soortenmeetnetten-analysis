...
verschillen_vorigjaar %>%
  ...
  kable(caption = "Verschil in aantal adulten voor opeenvolgende jaren",
      booktabs = TRUE,
      escape = FALSE,
      align = "lcccccccc") %>%
  kable_styling(font_size = 8) %>%
  column_spec(c(2:9), width = "1.5cm") %>%
  collapse_rows(1) %>%
  scroll_box(width = "700px", height = "100%")
```
...
```{r fighk4, fig.cap= "Proportie van de plassen en totaal aantal plassen voor de verschillende categorieën van de habitatkenmerken m.b.t. de vegetatie (meetnet rugstreeppad)"}

vegetatie %>%
  mutate(variabele = str_to_sentence(variabele),
         variabele = str_replace(variabele, "_", " ")) %>%
  ggplot(aes(x = variabele, y = prop_locatie, fill = waarde)) + 
  geom_col() +
  geom_text(aes(label = n), position = position_stack(vjust = 0.5), color = "white") +
