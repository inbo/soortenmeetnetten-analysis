...
analyseset_rugstreeppad_adult %>%
  write_vc(file = "analyseset_rugstreeppad_roepkoren", root = "../output/analyseset",
           sorting = c("locatie", "datum"))
...
trendmodel_iid <- inla(formula = aantal_adult_roepend ~ year_scaled  + 
                                f(locatie, 
                                  model = "iid", 
                                  hyper = list(theta = list(prior = "pc.prec", param = c(1, 0.05)))),
                       family = "nbinomial",
...
trendmodel_iid_no_offset <- inla(formula = aantal_adult_roepend ~ year_scaled  + 
                                f(locatie, 
                                  model = "iid", 
                                  hyper = list(theta = list(prior = "pc.prec", param = c(1, 0.05)))),
                       family = "nbinomial",
...
results_index_meetcyclus_rugstreeppad  <- bind_rows(results_index_meetcyclus_rugstreeppad %>% 
...
results_trend_rugstreeppad <- derive_trend(analyseset_species = analyseset_rugstreeppad_adult, trendmodel_nbinom = trendmodel_iid)

results_trend_rugstreeppad_no_offset <- derive_trend(analyseset_species = analyseset_rugstreeppad_adult, trendmodel_nbinom = trendmodel_iid_no_offset)
```
...
```{r}
plot(dispersion_check(indexmodel_paired_meetcyclus_iid))
```

#### Distribution

```{r}
plot(fast_distribution_check(indexmodel_paired_meetcyclus_iid))
```
...
```{r}
 ...
 precision_location <- indexmodel_paired_meetcyclus_iid$summary.hyperpar$mean[2]
...
results_index_meetcyclus <- results_index_meetcyclus_boomkikker %>% 
  bind_rows(results_index_meetcyclus_paired_rugstreeppad %>%
              mutate(type_model = "offset")) %>%
  bind_rows(results_index_meetcyclus_paired_rugstreeppad_nooffset %>%
              mutate(type_model = "no offset")) %>%
...
verschil_meetcycli <- results_index_meetcyclus %>%
  ...
  select(type_model, soort_nl, soort_wet, levensstadium, meetcyclus_ref, meetcyclus, klasse, mean, lcl_0.90, ucl_0.90) 
...
ggsave("../output/index_jaar_amfibieën.png",
       width = 9,
       height = 3.5)
```
...
results_trend <- results_trend_kamsalamander %>%
  ...
  bind_rows(results_trend_rugstreeppad)
...
ggsave("../output/trend_jaar_amfibieën.png",
       width = 5,
       height = 3)
...
```{r, eval = FALSE}
trend <- read_vc(root = "../output/results", file = "results_trendmodel_amfibieën") 

trend <- trend %>%
  mutate(type_model = ifelse(soort_nl == "Rugstreeppad", "offset", NA))

result_add <- results_trend_rugstreeppad_no_offset %>%
              mutate(type_model = "no offset") %>%
  group_by(soort_nl) %>%
  mutate(periode_tekst = str_c(jaar_min, " - ", jaar_max),
         n_jaar = jaar_max - jaar_min + 1,
  ...
  ungroup() %>%
  mutate(klasse = classification_tw(lcl_0.90, ucl_0.90, threshold_low = treshold_low, treshold_high =  treshold_high, reference = 0),
         mean_log = log(mean/100 + 1),
          lcl_0.90_log = log(lcl_0.90/100 + 1),
         ucl_0.90_log = log(ucl_0.90/100 + 1),
         soort_periode = str_c(soort_nl, jaar_min, jaar_max, sep = "_")) 

trend <- trend %>%
  bind_rows(result_add)
...
```{r}
trend %>%
  write_vc(root = "../output/results", file = "results_trendmodel_amfibieën", sorting = c("parameter", "soort_nl", "jaar_min", "type_model"), strict = FALSE)
```
```{r}
 results_index_jaar %>%
...
results_waic %>%
  write_vc(root = "../output/results", file = "results_waic_amfibieën", sorting = c("soort_nl"), strict = FALSE)
```

```{r output-2025, eval = FALSE}
...
trend_2024 <- read_vc(root = "../output/results",
                      file = "results_trendmodel_amfibieën")

results_trend <- trend_2024 %>%
  filter(soort_nl %in% c("Boomkikker", "Kamsalamander", "Rugstreeppad")) %>%
  filter(is.na(type_model) | (type_model == "no offset")) %>%
  bind_rows(results_trend_knoflookpad) %>%
  bind_rows(results_trend_vroedmeesterpad) %>%
  bind_rows(results_trend_vuursalamander)
...
trend <- results_trend %>%
  group_by(soort_nl) %>%
  mutate(soort_periode = str_c(jaar_min, " - ", jaar_max),
         n_jaar = jaar_max - jaar_min + 1,
         treshold_low = ifelse(parameter == "trend_average", 
                               round((exp(log(0.75)/(6 - 1)) - 1) * 100, 1),
                               -25),
  ...
  mutate(mean_log = log(mean/100 + 1),
          lcl_0.90_log = log(lcl_0.90/100 + 1),
         ucl_0.90_log = log(ucl_0.90/100 + 1),
         soort_periode = str_c(soort_nl, " (", jaar_min, "-", jaar_max,")"))
...
trend %>%
  ...
  geom_point(size = 7, alpha = 0.6) +
  geom_text(size = 4, colour = "white") +
  labs(y = "Gemiddelde jaarlijkse trend (%)", x = "Soort en periode monitoring") +
  #scale_y_continuous(breaks = c(-50, -25, 0, 33, 100)/100, labels = scales::percent, limits = c(-0.6, 1)) +
  scale_y_continuous(breaks = breaks_log, labels = labels_show, limits = c(min(breaks_log), max(breaks_log))) +
  coord_flip() +
  scale_color_manual(values = klasse_color) +
