...
Niet-meetnetlocaties meenemen in analyses?

Tellingen Buggenhoutbos aggregeren (rekening houdend met transectlengte) ?
...
dataset_heikikker <-  visits %>%
  filter(meetnet == "Heikikker") %>%
  left_join(select(covar, visit_id, bezoekvariabele, waarde), by = c("visit_id")) %>%
  left_join(locations, by = c("meetnet", "locatie")) %>%
