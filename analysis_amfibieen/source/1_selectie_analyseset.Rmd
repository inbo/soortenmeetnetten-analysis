...
```{r}
visits <- get_visits_smp(species_group = species_group, path = "../data") %>%
...
covar <- get_covariates_smp(path = "../data") 

covar_sample <- get_covariates_smp(linked_to = "sample", path = "../data")

counts <- get_counts_smp(path = "../data") %>%
  filter(jaar <= 2024)

locations <- get_locations_smp(only_active = FALSE, path = "../data") %>%
  st_drop_geometry() %>%
...
locaties_niet_actief <- dataset_roepkoren %>%
  group_by(meetnet, locatie,  primaire_soort) %>%
...
+ Niet actieve locaties waar soort al werd waargenomen
  + Merkske - Poel B46b: behoort niet tot de steekproef --> geen probleem
  + Zwin - Kleine vlakte 3: vervanging binnen cluster (door Zwin - Kleine vlakte 13) --> OK
...
locaties_niet_actief <- dataset_fuiken %>%
  filter(primaire_soort) %>%
  group_by(meetnet, locatie, is_sample, primaire_soort) %>%
  summarise(aantal = sum(aantal)) %>%
  ungroup() %>%
...
+ Niet actieve locaties waar soort al werd waargenomen
  + Dheide - Poel 1: Poel is verland, vervanging binnen cluster
  + Kooldries Hoofsweer - Poel 26: Poel is verland, vervanging binnen cluster
...
locaties_niet_actief <- dataset_roepkoren_knoflookpad %>%
  group_by(meetnet, locatie, type_aantal, primaire_soort) %>%
  summarise(totaal = sum(aantal)) %>%
...
+ Transect Hayesbos Noord stopgezet want het overlapt met Hayesbos zuid
...
habitatkenmerken_boomkikker_kamsalamander_wide <- habitatkenmerken_boomkikker_kamsalamander %>%
  ...
  mutate(ph = as.numeric(ph),
         oppervlakte_waterpartij = factor(oppervlakte_waterpartij, levels = c("onbekend", "niet bekeken/niet van toepassing","<10", "10-100", "101-250", "251-400", ">400"), ordered = TRUE),
         maximale_diepte = factor(maximale_diepte, levels = c("onbekend", "niet bekeken/niet van toepassing", "<0.5", "0.5-1", ">1", ">1.5"), ordered = TRUE),
         ...
         beschaduwing = factor(beschaduwing, levels = c("onbekend", "niet bekeken/niet van toepassing", "geen schaduw", "<30%", "30-60%", ">60%"), ordered = TRUE),
         ...
         waterkwaliteit = factor(waterkwaliteit, levels = c("onbekend", "niet bekeken/niet van toepassing", "plas verdwenen of volledig verland", "slecht", "middelmatig", "goed"), ordered = TRUE))
...
habitatkenmerken_rugstreeppad_vroedmeesterpad_wide <- habitatkenmerken_rugstreeppad_vroedmeesterpad %>%
  select(-eenheid) %>%
  pivot_wider(names_from = bezoekvariabele, values_from = waarde) %>%
  rename(oppervlakte_waterpartij = "oppervlakte waterpartij", maximale_diepte = "maximale diepte", aanwezigheid_vis = "aanwezigheid vis", permanente_waterkolom = "permanente waterkolom", drijvende_vegetatie = "drijvende vegetatie", verticale_vegetatie = "verticale vegetatie", ph = "pH (zuurtegraad)") %>%
  mutate(oppervlakte_waterpartij = factor(oppervlakte_waterpartij, levels = c("onbekend", "niet bekeken/niet van toepassing","<10", "10-100", "101-250", "251-400", ">400"), ordered = TRUE),
         maximale_diepte = factor(maximale_diepte, levels = c("onbekend", "niet bekeken/niet van toepassing", "<0.5", "0.5-1", ">1", ">1.5"), ordered = TRUE),
         aanwezigheid_vis = ifelse(aanwezigheid_vis %in% c("TRUE", "ja"), TRUE,
                                   ifelse(aanwezigheid_vis %in% c("FALSE", "nee"), FALSE, NA)),
         permanente_waterkolom = ifelse(permanente_waterkolom %in% c("TRUE", "ja"), TRUE,
                                   ifelse(permanente_waterkolom %in% c("FALSE", "nee"), FALSE, NA)),
         beschaduwing = factor(beschaduwing, levels = c("onbekend", "niet bekeken/niet van toepassing", "geen schaduw", "<30%", "30-60%", ">60%"), ordered = TRUE),
         drijvende_vegetatie = factor(drijvende_vegetatie, levels = c("onbekend", "niet bekeken/niet van toepassing", "0%", "1-25%", "26-50%", "51-75%", "76-100%"), ordered = TRUE),
         onderwatervegetatie = factor(onderwatervegetatie, levels = c("onbekend", "niet bekeken/niet van toepassing", "0%", "1-25%", "26-50%", "51-75%", "76-100%"), ordered = TRUE),
         verticale_vegetatie = factor(verticale_vegetatie, levels = c("onbekend", "niet bekeken/niet van toepassing", "0%", "1-25%", "26-50%", "51-75%", "76-100%"), ordered = TRUE))
```
