...
```{r}
analyseset_gb %>%
  mutate(prop_aanwezig_plant = n_plant_metei / (n_plant_metei + n_plant_zonderei)) %>%
...
```{r}
analyseset_gb %>%
  mutate(prop_aanwezig_knop = n_knop_metei / (n_knop_metei + n_knop_zonderei)) %>%
...
```{r}
analyseset_gb %>%
  mutate(n_planten = n_plant_metei + n_plant_zonderei) %>%
  ggplot(aes(x = jaar, y = n_planten, group = plot, colour = plot)) +
  geom_point() +
  geom_line() +
  facet_wrap(~gebied + subgebied) +
  ylab("aantal planten")
