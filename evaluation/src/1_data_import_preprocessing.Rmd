...
```{r}
# DO FIRST:
# export data from database en store in local folder: run 'get_data_meetnetten.Rmd' in repo 'https://github.com/inbo/soortenmeetnetten-queries'
# add jaardoelen for current year using the script 'update_jaardoelen.Rmd' in repo 'https://github.com/inbo/soortenmeetnetten-queries'
# update year_evaluation and versie in this code block

year_evaluation <- 2025
...
versie_time <- (bezoeken_orig %>%
  slice_max(n = 1,
            order_by = pmax(created, updated)) %>%
  mutate(date_newest = max(created, updated)))$date_newest %>%
...
bezoeken <- bezoeken_orig %>%
  ...
  mutate(taak = ifelse(is.na(taak), "Extra telling", taak),
         ...
         protocol = ifelse(protocol == "Rugstreeppad - Transect (v1)", "Rugstreeppad - Transect", protocol),
  ...
  mutate(bezoek_status = ifelse(meetnet == "Heikikker" & locatie %in% c("Winkelsbroek 1", "Het Goorken 1", "De Liereman west 1"), "telling_gvm", bezoek_status),
         jaardoel = ifelse(meetnet == "Rivierrombout" & str_detect(locatie, "Herentalsebaan brug"), TRUE, jaardoel))
...
metadata <- get_characteristics_smp() %>%
  ...
  mutate(protocol = ifelse(meetnet == "Lentevuurspin", "Lentevuurspin - Transect + Gebiedstelling", protocol),
         n_locaties = ifelse(meetnet == "Rivierrombout", 13, n_locaties)) %>% #locaties Herentalsebaan brug --> is_sample = TRUE
  filter(!(!is_sample & meetnet == "Rivierrombout"))
...
locaties <- locaties %>%
  mutate(is_sample = ifelse(meetnet == "Rivierrombout", TRUE, is_sample))
...
overzicht_taken_uitgevoerd <- bezoeken %>%
  ...
  mutate(winterseizoen = meetnet %in% c("Otter", "Vleermuizen - Wintertellingen", "Vermiljoenkever"),
         jaar = ifelse(winterseizoen, year(datum + 182), jaar)) %>%
  group_by(soortgroep,  meetnet, protocol, jaar, winterseizoen, locatie, taak, bezoek_status, jaardoel) %>%
...
overzicht_taken_uitgevoerd_wide <- overzicht_taken_uitgevoerd %>%
  ...
  select(soortgroep, meetnet, protocol, jaar, winterseizoen, locatie, is_sample, is_sample_history, is_active, taak, telling_ingevoerd, jaardoel, telling_cp, telling_gvm, telling_nvcp)

overzicht_locaties <- overzicht_taken_uitgevoerd_wide %>%
  group_by(soortgroep, meetnet, protocol, jaar, winterseizoen, locatie, is_sample, is_sample_history, is_active) %>%
  ...
  mutate(meetcyclus = ifelse(meetnet == "Kamsalamander", 
                             ...
                             ceiling((jaar - opstartjaar + 1 - winterseizoen)/duur_meetcyclus)),
         jaar_in_meetcyclus = (jaar - opstartjaar + 1) - (meetcyclus - 1) * duur_meetcyclus,
         jaar_in_meetcyclus = ifelse(winterseizoen, jaar_in_meetcyclus - 1, jaar_in_meetcyclus), #jaar 1 is pas afgerond als winterseizoen jaar1-jaar2 is afgewerkt
...
evaluatie_locaties <- overzicht_locaties %>%
  mutate(n_bezoeken_doel = ifelse(meetnet == "Bosbeekjuffer" & jaar == 2018, 2, n_bezoeken_doel),
         n_bezoeken_doel = ifelse(meetnet == "Heikikker" & jaar == 2024, 1, n_bezoeken_doel)) %>%
  mutate(voldoende_geteld_strikt = ifelse((aantal_taken_cp + aantal_tellingen_gvm) >= n_bezoeken_doel, 1, 0),
        ...
        prop_geteld_loc = ifelse(is_sample_history, 
                                  pmin(aantal_tellingen_jaardoel/ n_bezoeken_doel, 1),
                                  pmax(pmin(aantal_tellingen_cp / n_bezoeken_doel, 1), aantal_tellingen_gvm >= 1)),
        prop_data_loc = pmin(aantal_tellingen_cp/n_bezoeken_doel, 1))
...
evaluatie_locaties <- evaluatie_locaties %>%
  ...
  mutate(temp1 = rank(desc(voldoende_geteld)) * 100  + rank(jaar),
         temp2 = rank(desc(voldoende_data)) * 100  + rank(jaar),
        ...
        prop_geteld_max = max(prop_geteld_loc),
        prop_data_max = max(prop_data_loc)) %>%
  ungroup() %>%
...
hulpcalc <- evaluatie_locaties %>%
  filter(soortgroep != "0") %>%
  distinct(is_sample_history, meetnet, protocol, meetcyclus, locatie, prop_geteld_max, prop_data_max) %>%
  group_by(is_sample_history, meetnet, protocol, meetcyclus) %>%
  summarise(prop_geteld_mn_cyclus = sum(prop_geteld_max),
            prop_data_mn_cyclus = sum(prop_data_max)) %>%
  ungroup()

#overzicht per meetnet
overzicht_meetnetten <- evaluatie_locaties %>%
  filter(soortgroep != "0") %>%
  group_by(is_sample_history, soortgroep, meetnet, protocol, jaar, winterseizoen, duur_meetcyclus, meetcyclus, jaar_in_meetcyclus) %>%
  summarise(aantal_tellingen_cp = sum(aantal_tellingen_cp),
            ...
            prop_geteld_mn = sum(prop_geteld_loc),
            prop_data_mn = sum(prop_data_loc)) %>%
  ungroup() %>%
  ...
  ungroup() %>%
  left_join(hulpcalc, by = c("is_sample_history", "meetnet", "protocol", "meetcyclus")) %>%
...
bezoeken_planten <- bezoeken %>% 
  ...
  filter(jaardoel |
           locatie %in% c("Rielens Gebroekt (c5-17-41)", "Gulke Putten (d2-13-22+ d2-13-24)")) %>%
  left_join(locaties_metadata, by = c("meetnet", "locatie")) %>%
...
#write.csv2(overzicht_todo_planten, "../output/planten_todo_meetcyclus3.csv", row.names = FALSE)
...
burchten_das <- data_extern %>%
  ...
  bind_rows(data.frame(jaar = 2024,
                       locatie = "burcht_dummy",
                       gebied = c("Voeren"),
                       aantal_tellingen_cp = 0)) %>%
  mutate(soortgroep = "zoogdieren",
         ...
         meetcyclus = ifelse(jaar <= 2018, 1, 
                             ifelse(jaar <= 2021, 2, 3)),
...
overzicht_meetnet_das <- burchten_das %>%
  ...
  mutate(is_sample_history = 1,
        cyclusdoel = ifelse(gebied == "Haspengouw", 350, 100),
        duur_meetcyclus = 3) %>%
  arrange(gebied)
...
doelbereik_cyclus_totaal_zdv <- overzicht_meetnetten_all_doel %>%
  filter(jaar_in_meetcyclus == duur_meetcyclus) %>%
  filter(!is.na(doelbereik_cyclus)) %>%
  filter(gebied != "Voeren" | is.na(gebied)) %>%
  group_by(jaar) %>%
  summarise(doelbereik_totaal = sum(cyclusdoel * doelbereik_cyclus_m) / sum(cyclusdoel),
            meetnetten_evaluatie = str_c(meetnet, collapse = "; ")) %>%
  ungroup()
...
bezoeken_dubbels <- bezoeken %>%
  left_join(locaties_metadata %>%
              select(meetnet, locatie, is_sample),
            by = c("meetnet", "locatie")) %>%
  group_by(soortgroep, meetnet, protocol, jaar, locatie, is_sample, datum) %>%
...
taak_locaties <- taken %>%
  filter(werkpakket == "EUROBATS 2023-2024") %>%
...
write_csv2(overzicht_wintertellingen, "../output/overzicht_wintertellingen_2023_2024.csv")
...
Het merendeel van de data komt uit de databank gekoppeld aan de meetnetten.be applicatie.
We maken gebruik van een data-export met als tijdstip van het recentste ingevoerde of aangepaste bezoek: `r versie_time`.

Voor een aantal meetnetten is het nog niet mogelijk om de data in te voeren via meetnetten.be.
In deze gevallen wordt de data jaarlijks aangeleverd aan het INBO, meestal onder de vorm van een excel-bestand. 

Voor elk meetnet worden jaardoelen gedefinieerd.
Dit zijn afspraken over de te leveren inspanning uitgedrukt in aantal tellingen of het aantal locaties met een voldoende aantal tellingen (conform het protocol). 
Voor de evaluatie van de jaardoelen rekenen we de tellingen conform protocol en de tellingen waarvoor geen veldwerk mogelijk is mee. 
Voor de verkenning van de aantallen selecteren we enkel de tellingen conform protocol. 

De R-code die gebruikt werd voor de evaluatie vind je in [deze repository](https://github.com/inbo/soortenmeetnetten-analysis/tree/master/evaluation).
