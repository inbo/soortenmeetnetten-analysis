...
analyseset_rugstreeppad_adult <- analyseset_rugstreeppad_est %>%
  ...
  dplyr::filter(type_telling_est == "avondtelling") %>%
  dplyr::filter(levensstadium == "adult") %>%
  group_by(locatie) %>%
...
library(INLA)
test <- data.frame(aantal = c(5, 6, 9, 12, 46, 15, 26),
                   jaar = factor(2020:2026))

model_inla <- inla(formula = aantal ~ jaar,
                   data = test)
```
