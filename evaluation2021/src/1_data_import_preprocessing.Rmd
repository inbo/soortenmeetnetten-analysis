...
```{r}
# bij otter worden sommige bezoeken op sublocatie niveau ingevoerd

bezoeken_ <- bezoeken %>%
  mutate(sublocatie = ifelse(locatie %in% sublocaties_otter$locatie, locatie, NA)) %>%
  left_join(sublocaties_otter, by = c("meetnet", "locatie")) %>%
  mutate(locatie = ifelse(is.na(hoofdlocatie), locatie, hoofdlocatie)) %>%
  select(-hoofdlocatie)
```
...
# path <- file.path(fileman_up("soortenmeetnetten-data"), "output/contole_plantendata/")
# 
# path <- "C:/Users/toon_westra/Rprojecten/soortenmeetnetten-data/output/controle_plantendata"
# 
# aantallen_planten_extern <- read_vc(file = "data_waarnemingen_migratie2021", root = path) %>%
...
#             code_all = str_c(floroncode, collapse = "-")) %>%
#   ungroup() %>%
#   mutate(bron = "waarnemingen.be",
#          track = FALSE) %>%
#   inner_join(select(bezoeken, visit_id, hoofdteller), by = "visit_id")
# 
...
write.csv2(overzicht_locaties_planten, "../output/plantenmeetnetten_locaties_meetcyclus2.csv", row.names = FALSE)
...
#write.csv2(overzicht_alle_locaties_planten, "../output/plantenmeetnetten_locaties_meetcyclus2.csv", row.names = FALSE)
...
data_extra <- read.csv2("../data/overzicht_soorten_buiten_mn.csv", stringsAsFactors = FALSE) %>%
...
bezoeken_buiten_telperiode <- bezoeken %>%
  ...
  arrange( desc(verschil_dagen_abs)) %>%
    select(soortgroep, meetnet, protocol, telperiode, locatie,  bezoek_status_detail, datum, visit_id, verschil_dagen) %>%
...
counts_bezoek <- bezoeken %>%
  filter(voor_analyse) %>%
  semi_join(locaties_actief, by = c("meetnet", "locatie")) %>%
  select(soortgroep, meetnet, protocol, locatie, jaar, datum, visit_id) %>%
