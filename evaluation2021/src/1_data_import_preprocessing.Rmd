...
bezoeken <- bezoeken_orig %>%
  ...
  filter(jaar >= 2016) %>%
  mutate(locatie = ifelse(visit_id %in% c("53149", "53150", "53151"), "Dheide - Poel 4", locatie)) %>%
...
aantallen <- get_counts_smp(file = "raw/aantallen_20240125")
...
locaties_metadata <- locaties %>%
  ...
  mutate(is_active = ifelse(meetnet == "Kamsalamander" & locatie == "Dheide - Poel 2", FALSE, is_active),
         is_sample = ifelse(meetnet == "Kamsalamander" & locatie == "Dheide - Poel 2", FALSE, is_sample),
         is_sample = ifelse(meetnet == "Kamsalamander" & locatie %in% c("Dheide - Poel 3", "Schuddebeurze - Poel 1"), TRUE, is_sample)) %>% 
...
overzicht_taken_uitgevoerd <- bezoeken %>%
  # mutate(jaardoel = ifelse(meetnet == "Bosbeekjuffer" & locatie %in% c("Rosdel", "Afwateringskanaal", "Kindernouwbeek", "Prinsenpark"), TRUE, jaardoel)) %>% 
  # mutate(jaardoel = ifelse(meetnet == "Roestbruine kniptor" & locatie %in% c("Kartuizerduinen"), TRUE, jaardoel)) %>%
...
aantallen_planten_extern <- read_csv2("C:/Rprojecten/soortenmeetnetten-queries/output/controle_plantendata/plantenmeetnetten_migratie_2023.csv") %>%
...
evaluatie_locaties_vleermuizen <- evaluatie_locaties %>%
  semi_join(taak_locaties, by = c("meetnet", "locatie")) %>%
  filter(jaar == year_evaluation)



doelbereik <- sum(evaluatie_locaties_vleermuizen$voldoende_geteld)/n_distinct(taak_locaties$locatie)
