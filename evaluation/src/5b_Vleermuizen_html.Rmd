...
bezoeken_afwijkingen <- bezoeken %>% 
  ...
  select(link, meetnet,  datum, locatie, eurobats =  is_sample, "bezoek status" = bezoek_status_detail, opmerkingen = notes) %>%
  ...
  bezoeken_afwijkingen %>%
    datatable(
            ...
            options = list(
              ...
              deferRender = TRUE,
              autoWidth = TRUE,
              columnDefs = list(list(width = '40%', targets = c(6)))
              )
            )
...
bezoeken_dubbels_soortgroep <- bezoeken_dubbels %>%
  filter(soortgroep %in% soortgroep_sel) %>%
  select(meetnet, protocol, locatie, eurobats = is_sample, datum, bezoeken = n_bezoeken_datum, "zelfde begintijdstip" = start_time_identiek) %>% 
