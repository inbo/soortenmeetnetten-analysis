...
Bij de selectie van de VPS [@de_knijf_monitoring_2014] ging er bijzondere aandacht naar de Kempense heidelibel. Deze soort staat niet vermeld op de bijlage van de HRL, maar staat wel als Kwetsbaar aangeduid op de eerste Europese Rode Lijst van libellen [@kalkman_european_2010] In de nieuwe Europese Rode Lijst van libellen [@deknijf_2024], behoudt de Kempense heidelibel zijn status als Kwetsbaar. Maar ook de Maanwaterjuffer en de Speerwaterjuffer werden allebei beoordeeld als Kwetsbaar.
...
meetnet_characteristics <- get_characteristics_smp(species_group = "libellen") %>%
  ...
  mutate(protocol = ifelse(meetnet %in% c("Beekrombout", "Rivierrombout"), "Transecttelling Larvenhuidjes",
...
Deze wijzigingen hebben dus nog geen effect op de resultaten die we in dit rapport tonen, maar we lichten ze alvast toe.
