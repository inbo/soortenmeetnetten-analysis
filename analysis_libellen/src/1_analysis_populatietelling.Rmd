...
results_indexmodel <- models_inlabru %>%
  transmute(index = map2(data, indexmodel, derive_index_inlabru),
            max_count = map2(data, indexmodel, derive_max_count_inlabru),
            diff_years = map(data, derive_diff_years_inla))
...
bind_rows(results_index, 
  ...
  write_vc(file = "results_indexmodel", 
         ...
         sorting = c("parameter", "soort_nl", "jaar", "jaar_ref"))
...
idd_checked_plot <- idd_checked %>%
  group_by(meetnet) %>%
  mutate(x_c = sim_idd - mean(sim_idd),
         y = exp(x_c + log(1))) %>%
  ungroup()
...
# Vergelijking met analyseset voortgangsrapport

```{r}
analyseset_voortgang <- read.csv2(here("data","analyseset_voortgang.csv")) %>%
  group_by(visit_id) %>%
  mutate(n_visit_id = n()) %>%
  ungroup()


analyseset_2018 <- counts_analysis %>%
  filter(jaar <= 2018) %>%
  filter(meetnet != "Hoogveenglanslibel") %>%
  filter(meetnet != "Speerwaterjuffer")

analyseset_voortgang_extra_data <- analyseset_voortgang %>%
  anti_join(analyseset_2018, by = "visit_id")

analyseset_2018_extra_data <- analyseset_2018 %>%
  anti_join(analyseset_voortgang, by = "visit_id")
```


```{r}
set.seed(47830)
analyseset_species <- analyseset_voortgang %>%
  filter(meetnet == "Maanwaterjuffer") %>%
  filter(jaar <= 2018) %>%
  mutate(aantal = n_individuen,
         soort_wet = "x")
  
model_soort_voortgang <- analyseset_soort_voortgang %>%
  fit_indexmodel_nbinom_inlabru() 

set.seed(47830)
index_soort_2018_voortgang <- derive_index_inlabru(analyseset_soort_voortgang, model_soort_voortgang) 
set.seed(47830)
index_soort_2018_voortgang_1 <- derive_index_inlabru(analyseset_soort_voortgang, model_soort_voortgang) 
```
