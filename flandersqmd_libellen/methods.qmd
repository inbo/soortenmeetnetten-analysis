...
output_html <- knitr::is_html_output()
...
#| tbl-cap: "Overzicht van de karakteristieken van de libellenmeetnetten: aantal bezoeken per jaar, telperiode, aantal locaties en duur van de meetcyclus"
...
meetnet_characteristics_table <- meetnet_characteristics %>%
  ...
  select(Meetnet = meetnet, Veldprotocol =  protocol, "Aantal bezoeken (per jaar)" = bezoeken,
         Telperiode = telperiode, "Selectie locaties" = type, "Aantal locaties" = n_locaties ,
         "Duur meetcyclus (jaar)" = duur_meetcyclus) %>%
  unique()
...
In @fig-inspanninghtml zien we de evolutie van de tellingen in de tijd voor alle libellenmeetnetten samen.
...
In @fig-inspanning-pdf zien we de evolutie van de tellingen in de tijd voor alle libellenmeetnetten samen.
Het aantal getelde locaties nam toe in de periode 2016 tot 2018, wat te verklaren is doordat er jaarlijks een aantal nieuwe meetnetten werden opgestart.
Sinds 2018 zijn alle libellenmeetnetten opgestart en zien we een stagnatie. De ogenschijnlijke lichte achteruitgang kan verklaard worden doordat bepaalde soorten en locaties in een driejarige cyclus worden opgevolgd.
Het ziet er naar uit dat alle tellers hun locaties jaarlijkse blijven opvolgen en dus goed volhouden.

```{r}
#| label: fig-inspanning-pdf
#| fig-cap: "Monitoringinspanning voor alle libellenmeetnetten samen"

monitoring_effort_soortgroep %>%
  ggplot(aes(x = jaar, y = aantal, colour = monitoringsinspanning)) +
  geom_point() +
  geom_line() +
  ylim(0, NA) +
...
trend_klasses %>%
  arrange(klasse) %>%
  select(Code = klasse, Klasse = klasse_naam, Beschrijving = klasse_beschrijving) %>%
  kable(align = c("c", "l", "l"),
        booktabs = TRUE,
        escape = FALSE) %>%
  kable_styling() %>%
