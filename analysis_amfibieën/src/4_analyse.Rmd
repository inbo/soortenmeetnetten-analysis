...
analyseset_roepkoren_boomkikker <- analyseset_roepkoren_boomkikker %>%
  ...
  mutate(year_scaled = jaar_scaled)
...
fun = function(...) {exp(meetcyclus2021_2023)}
  ...
  model_inla.samples <- inla.posterior.sample(1000, indexmodel_iid, seed = 25654, num.threads="1:1")
...
analyseset_fuiken_totaal <- analyseset_fuiken_totaal %>%
  mutate(cluster2 = cluster,
         meetcyclus_scaled = ifelse(jaar <= 2020, 0, 1))
...
trend <- results_trend_kamsalamander %>%
  ...
  bind_rows(results_trend_kamsalamander2) %>%
  group_by(soort_nl) %>%
