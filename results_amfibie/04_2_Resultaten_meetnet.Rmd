...
Meer achtergrondinformatie over de opmars van deze soort in Vlaanderen is te vinden in @engelen_2024a en @engelen_2024b.
...
Wanneer we echter opeenvolgende jaren vergelijken zien we dat de aantallen in de laaste drie jaren stabiel zijn gebleven.
...
We tonen hier enkel de resultaten van de analyse van het aantal roepkoren (zie paragraaf \@ref(h:analyserugstreeppad)).
...
De resultaten van de analyse zijn deze gebaseerd op een deel van de meetnetlocaties en zijn dus niet volledig representatief voor Vlaanderen.
...
result_indexmodel_cyclus_table %>%
  filter(soort_nl == m) %>%
  filter(type_model == "offset" | is.na(type_model)) %>%
  mutate(soort_show = str_c(soort_nl, " ", levensstadium)) %>%
  ggplot(aes(y = mean_log, ymin = lcl_0.90_log, ymax = ucl_0.90_log, x = meetcyclus, label = klasse, colour = klasse)) +
  geom_errorbar(width = 0, size = 20 * 2.58 / 3, alpha = 0.3) +
  geom_hline(yintercept = 1, linetype = 2) +
...
Over de periode `r trend_average_meetnet$periode_tekst` vertoont de `r m` een gemiddelde jaarlijkse trend in aantal roepkoren van `r round(trend_average_meetnet$mean, 0)` % met een 90% betrouwbaarheidsinterval tussen `r round(trend_average_meetnet$lcl_0.90, 0)` % en `r round(trend_average_meetnet$ucl_0.90, 0)` %.
De jaarlijke trend is onbekend als gevolg van het brede betrouwbaarheidsinterval.
Maar als we de periode 2022-2024 vergelijken met de periode 2019-2021 zien we wel een (nipt) significante toename in aantel roepkoren.

Gebieden met een sterke toename in het aantal getelde roepkoren zijn o.a.
De Liereman, De Maten, Ravelse Bergen en De Westhoek.
In de gebieden Bergerven, het westelijk deel van De Teut en Noordelijk eiland was er een duidelijke afname in het aantal getelde roepkoren.
...
Over de periode `r trend_average_meetnet$periode_tekst` vertoont de `r m` een gemiddelde jaarlijkse trend van `r round(trend_average_meetnet$mean, 0)` % met een 90% betrouwbaarheidsinterval tussen `r round(trend_average_meetnet$lcl_0.90, 0)` % en `r round(trend_average_meetnet$ucl_0.90, 0)` %.
`r ifelse(trend_average_meetnet$klasse_naam == "onbekend", str_c("De trend is dus **onbekend** (", trend_average_meetnet$klasse_output, ").") , str_c("We kunnen dus spreken van een **", trend_average_meetnet$klasse_naam, "** (", trend_average_meetnet$klasse_output, ") van de ", str_to_lower(m), " in deze periode."))` Het relatief lager niveau van 2016 is wellicht deels toe te schrijven aan de onervarenheid van de tellende vrijwilligers. Natte zomers geven echter aanleiding tot groter reproductiesucces en aanwas van de populaties. De toename van 2021 naar 2022 is ook in die zin te begrijpen. In de daaropvolgende jaren blijven de aantallen van deze langlevende soort verhoogt.
