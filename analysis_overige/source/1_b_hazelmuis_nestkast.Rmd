...
```{r}
hazelmuis_nestbuis_bezoek %>%
  group_by(meetnet, soort_nl, levensstadium, activiteit, observation_type, code, description) %>%
...
```{r}
hazelmuis_nestbuis_bezoek$soort_nl %>%
  unique()
```


```{r, fig.width= 9,fig.height=8, fig.cap= "Voorkomen per bezoek"}
hazelmuis_nestbuis_bezoek %>%
  mutate(description = ifelse(observation_type == "versheid muizensporen", description,
         ...
         type = factor(type, levels = c("adult", "subadult", "adult/subadult", "juveniel", "nest", "keutels", "vraatsporen"))) %>%
  ggplot(aes(x = type, fill = description)) +
  geom_bar() +
  facet_wrap(~soort_nl, ncol = 2) +
  coord_flip() +
...
```{r, fig.width= 10, fig.cap= "Voorkomen individuen per bezoek"}
hazelmuis_nestbuis_bezoek %>%
  mutate(description = ifelse(observation_type == "versheid muizensporen", description,
                              ifelse(aantal  > 0, "aanwezig", "afwezig")),
         type = ifelse(observation_type == "versheid muizensporen", activiteit, levensstadium)) %>%
  filter(levensstadium != "unknown") %>%
  ggplot(aes(x = type, fill = description)) +
  geom_bar() +
...
hazelmuis_nestbuis_occurence %>%
  ...
  geom_bar() +
  labs(y = "Aantal nestbuizen", fill = "Aantal tellingen per nestbuis")
```
...
hazelmuis_nestbuis_occurence %>%
  ...
  geom_bar() +
  facet_wrap(~ activiteit + soort_nl, ncol = 2) +
  theme(legend.position = "bottom")
```
...
indexmodel_iid_0_bis <- inla(formula = aanwezig ~ 0 + fjaar +
                                f(sublocatie, 
                                  model = "iid", 
...
prop_model <- tibble(year_scaled = 0:4,
                     trend_logit = trend,
                     intercept_logit = intercept) %>%
  mutate(logodds = intercept + year_scaled * trend_logit,
         odds = exp(logodds),
         prop = plogis(logodds))

prop_model %>%
  select(year_scaled, logodds, odds, prop) %>%
  pivot_longer(cols = c(logodds, odds, prop),
               names_to = "parameter",
               values_to = "estimate") %>% 
  ggplot(aes(x = year_scaled, y = estimate, group = parameter, colour = parameter)) +
  geom_line()
