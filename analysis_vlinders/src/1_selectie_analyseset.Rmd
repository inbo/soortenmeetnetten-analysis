...
locatie_detail <- get_locations_smp(path = "../data") %>%
  st_drop_geometry() %>%
...
counts_all <- get_counts_smp(species_group = species_group, path = "../data")
...
counts_gb <- read_vc(file = "raw/aantallen_gentiaanblauwtje",
                     root = "../data") %>%
...
covars <- get_covariates_smp(species_group = species_group, path = "../data") %>%
  filter(protocol %in% protocol_selection) %>%
  filter(meetnet != "Gentiaanblauwtje") %>%
  select(-eenheid) %>%
...
count_period <- get_characteristics_smp(path = "../data") %>%
...
counts_sublocation_selection <- visits_selection %>%
  select(soortgroep, meetnet, protocol, generatie, locatie, jaar, datum, doy, doy_scaled, doy_min, doy_max, doy_mid,  visit_id, n_obs, voor_analyse, is_active, is_sample, bezoek_status, bezoek_status_oud, notes) %>%
...
analyseset_gebied <- counts_sublocation_selection_sample %>%
  ...
  filter(type_aantal == "totaal gezien") %>%
  filter(primaire_soort) %>%
...
write_vc(analyseset_gebied, file = str_c("analyseset_", name_analysis0), 
...
analyseset_gb <- counts_gb %>%
  group_by(meetnet, locatie, gebied, subgebied, plot, visit_id, datum, jaar, bezoek_status, notes, plant, stengel, knop) %>%
  summarise(n_ei_knop = sum(aantal)) %>%
  ungroup() %>%
  group_by(meetnet, locatie, gebied, subgebied, plot, visit_id, datum, jaar, bezoek_status, notes, plant) %>%
  summarise(n_ei_plant = sum(n_ei_knop),
            n_knop_metei = sum(n_ei_knop > 0),
            n_knop_zonderei = sum(n_ei_knop == 0)) %>%
  ungroup() %>%
  group_by(meetnet, locatie, gebied, subgebied, plot, visit_id, datum, jaar, bezoek_status, notes) %>%
