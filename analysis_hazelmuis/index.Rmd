...
visits_hazelmuis <- get_visits_smp(file = "raw/bezoeken_20231213") %>%
  filter(protocol == "Hazelmuis - Transect") %>%
  select(visit_id, bezoek_status, validatie)

aantal_nesten <- get_counts_smp(file = "raw/aantallen_20231213") %>%
  filter(protocol == "Hazelmuis - Transect") %>%
...
visits_nestbuizen <- get_visits_smp(file = "raw/bezoeken_20231213") %>%
  filter(protocol == "Hazelmuis - Nestbuizen") %>%
  select(visit_id, bezoek_status, validatie) %>%
  mutate(visit_id = as.character(visit_id))

aantal_nestenbuizen <- read_vc(root = fileman_up("soortenmeetnetten-queries"), file = "raw/aantal_hazelmuis_nestbuizen_20231218") %>%
