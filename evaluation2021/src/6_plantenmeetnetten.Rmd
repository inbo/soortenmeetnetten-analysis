...
overzicht_planten %>%
  ...
  select(jaar, meetnet,
         ...
         "met track" = voldoende_geteld_track) %>%
  kable(align = "c",
        caption = "Tellingen voor alle plantenmeetnetten samen in 2022 en 2023") %>%
   kable_styling() %>%
...
aantallen_planten_extra <- read_csv2("C:/Rprojecten/soortenmeetnetten-queries/output/controle_plantendata/plantenmeetnetten_extra_2023.csv") %>%
  st_as_sf(coords = c("x", "y"), crs = 31370) %>%
  st_transform(4326) %>%
...
Onderstaande kaart geeft de tellingen voor 2022. We onderscheiden:
