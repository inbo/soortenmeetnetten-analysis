...
locaties_kamsalamander <- locaties %>%
  ...
  st_join(steekproefkader_kamsalamander) %>%
  st_drop_geometry()
...
kamsalamander_geteld <- evaluatie_locaties %>%
  ...
  summarise(voldoende_data = ifelse(sum(voldoende_data) > 0, 1, 0)) %>%
  ungroup() %>%
...
overzicht_kamsalamander <- locaties_kamsalamander %>%
  ...
  mutate( voldoende_data = ifelse(is.na(voldoende_data), 0, voldoende_data)) %>%
  spread(key = "protocol_short", value = "voldoende_data") %>%
  mutate(afgewerkt = scheppen == 1 & fuiken == 1)
...
Dheide Poel 4 vervangt Dheide Poel 3!
