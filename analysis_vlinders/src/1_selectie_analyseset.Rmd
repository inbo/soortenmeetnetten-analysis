...
counts_all <- bind_rows(
  get_counts_smp(species_group = species_group, count_aggregation = "individuals"),
)
...
counts_sublocation_selection <- visits_selection %>%
  ...
  left_join(select(counts, visit_id, sublocatie, soort_nl, soort_wet, aantal, niet_geteld, primaire_soort, plant, stengel, knop), by = "visit_id") %>%
...
write_vc(analyseset_gb, file = str_c("analyseset_", name_analysis), 
       root = here(str_c("analysis_vlinders/output/temp")), 
       sorting = c("meetnet", "locatie", "visit_id"), 
       strict = FALSE)
...
names_analysis <- c(name_analysis1, name_analysis2, name_analysis3)
...
hashes <-
    ...
    mutate(version = Sys.Date(),
           ...
           name_analysis = str_sub(filename, start = 12, end = -5),
